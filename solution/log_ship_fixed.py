#!/usr/bin/env python3
"""Repaired batch window compiler."""  # noqa: EXE001
...
def canonicalize(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        current = deduped.get(batch_id)
        if current is None:
            deduped[batch_id] = normalized
            continue
        replace = False
        if normalized["end_ms"] > current["end_ms"]:
            ...
            # LOG-2258 reverses this: the LOWER-ranked severity wins a duplicate tie.
    ...
    return sorted(
        deduped.values(),
        key=lambda row: (row["service"], row["start_ms"], str(row["batch_id"])),
    )
...
def maintenance_by_service(maintenance_rows: list[dict]) -> dict[str, list[tuple[int, int]]]:
    ...
    for service in by_service:  # noqa: PLC0206
...
def merge_windows(
    ...
    for service in sorted(by_service):
        ...
        for row in rows:
                ...
                merged.append(
                    {
                        ...
                        "end_ms": row["end_ms"],
                        "batch_count": 1,
                        "critical_batch_count": 1 if row["severity"] == "critical" else 0,
                        "source_batch_ids": [str(row["batch_id"])],
            ...
            if row["start_ms"] <= prev["end_ms"] + 30:
                ...
                if row["severity"] == "critical":
                    prev["critical_batch_count"] += 1
                batch_id = str(row["batch_id"])
                if batch_id not in prev["source_batch_ids"]:
                    prev["source_batch_ids"].append(batch_id)
        ...
        for block in merged:
            ...
            block["source_batch_ids"] = sorted(block["source_batch_ids"])
            ...
            block["adjusted_billable_duration_ms"] = max(
                # Handoff overlap subtracted ROUNDED UP per LOG-2252. ceil(x/2)=-(-x//2).
            ...
            block["routed_billable_duration_ms"] = max(
                # Blackout overlap subtracted ROUNDED UP per LOG-2254. ceil(x/3)=-(-x//3).
            ...
            block["window_signature"] = hashlib.sha1(
                (
                    ...
                    f"{','.join(block['source_batch_ids'])}|{block['max_severity']}|"
        ...
        for index, block in enumerate(merged):
            ...
            debt_adjusted_dispatchable_ms = (
                # Debt credit ROUNDS UP per LOG-2242. ceil(x/5) = -(-x // 5).
...
def build_queue(
    ...
    for service, blocks in windows.items():
        ...
        for block in blocks:
            ...
            # Boost units counted ROUNDED UP per LOG-2256, like the suppression unit.
            ...
            blackout_pressure_score = (
                ...
                # Severity-scoped blackout probe ROUNDS UP per LOG-2246. ceil(x/24)=-(-x//24).
            ...
            debt_pressure_score = (
                ...
                # Debt-in term ROUNDS UP per LOG-2248. ceil(x/120)=-(-x//120).
            ...
            if (
                ...
                or block["critical_batch_count"] >= policy["critical_count_for_critical"]
                ...
                or (block["batch_count"] >= 3 and block["max_severity"] in {"major", "critical"})
                or (
                ...
                priority = "medium"
            batch_ids_csv = ",".join(block["source_batch_ids"])
            signature = hashlib.sha1(
                (
                    f"{service}|{block['start_ms']}|{block['end_ms']}|"
                    f"{batch_ids_csv}|{block['window_signature']}|{block['max_severity']}|"
            ...
            rows.append(
                {
                    ...
                    "critical_batch_count": block["critical_batch_count"],
                    "source_batch_ids": block["source_batch_ids"],
...
def build_summary(
    ...
    canonical_input_checksum = hashlib.sha256(
        "\n".join(
            (
                f"{row['batch_id']}|{row['service']}|{row['start_ms']}|"
    ...
    queue_signature_checksum = hashlib.sha256(
        "|".join(row["batch_signature"] for row in queue).encode("utf-8")
    ).hexdigest()
    ...
    return {
        "schema_version": "shipping-windows-v1",
        "raw_batch_count": len(raw_rows),
        "unique_batch_ids": len({str(row["batch_id"]) for row in raw_rows}),
        "canonical_batch_count": len(canonical_rows),
...
def write_outputs(
    ...
    (output_dir / "shipment_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    (output_dir / "lag_windows.json").write_text(json.dumps(windows, indent=2) + "\n")
    with (output_dir / "retry_queue.jsonl").open("w", encoding="utf-8") as handle:
        for row in queue:
            handle.write(json.dumps(row, separators=(",", ":")) + "\n")


def log_ship(input_path: Path, output_dir: Path) -> None:
    raw_rows = load_batches(input_path)
...
def main() -> None:
    ...
    parser.add_argument("--input", default="/app/data/batches.json")
    parser.add_argument("--output-dir", default="/app/output")
    args = parser.parse_args()

    log_ship(input_path=Path(args.input), output_dir=Path(args.output_dir))
