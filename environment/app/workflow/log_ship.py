#!/usr/bin/env python3
"""Outage window compiler: builds service disruption windows and the responder queue."""  # noqa: EXE001
...
def dedupe_batchs(rows: list[dict]) -> list[dict]:
    deduped: dict[str, dict] = {}
    for row in rows:
        batch_id = str(row["batch_id"])
        if batch_id not in deduped:
            deduped[batch_id] = dict(row)
...
def main() -> None:
    ...
    canonical_rows = dedupe_batchs(raw_rows)
