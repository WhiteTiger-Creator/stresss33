"""Verifier tests for batch compiler hard task."""
...
SUMMARY_PATH = OUTPUT_DIR / "shipment_summary.json"
WINDOWS_PATH = OUTPUT_DIR / "lag_windows.json"
QUEUE_PATH = OUTPUT_DIR / "retry_queue.jsonl"

PIPELINE = Path("/app/workflow/log_ship.py")
ORIGINAL_PIPELINE = Path("/app/workflow/.log_ship.original")
INPUT_PATH = Path("/app/data/batches.json")
MAINTENANCE_PATH = Path("/app/data/maintenance_windows.json")
POLICY_PATH = Path("/app/data/response_policies.json")
EXCEPTIONS_PATH = Path("/app/data/shipping_exceptions.json")
...
def test_cli_exists():
    """The batch compiler exists at its operational path."""
...
def test_output_dir_contains_exactly_three_files():
    ...
    assert names == {"shipment_summary.json", "lag_windows.json", "retry_queue.jsonl"}


def test_primary_summary_exact_fixture(summary: dict):
    """shipment_summary.json from the primary input matches its canonical SHA-256 exactly."""
...
def test_primary_windows_exact_fixture(windows: dict[str, list[dict]]):
    """lag_windows.json from the primary input matches its canonical SHA-256 exactly."""
    assert _stable_json_hash(windows) == FIXTURE["primary_windows_sha256"]


def test_primary_queue_exact_fixture(queue_rows: list[dict]):
    """retry_queue.jsonl from the primary input matches its canonical SHA-256 exactly."""
...
def test_alternate_input_exact_fixture():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert result.returncode == 0, result.stderr
        assert _stable_json_hash(_load_json(out / "shipment_summary.json")) == FIXTURE[
            "alternate_summary_sha256"
        ]
        assert _stable_json_hash(_load_json(out / "lag_windows.json")) == FIXTURE[
            "alternate_windows_sha256"
        ]
        assert _stable_jsonl_hash(_load_jsonl(out / "retry_queue.jsonl")) == FIXTURE[
            "alternate_queue_sha256"
        ]


def test_summary_schema_and_order(summary: dict):
    """shipment_summary.json carries exactly the contract keys with required orderings."""
    ...
    assert summary["schema_version"] == "shipping-windows-v1"
...
def test_window_shape_and_sorting(windows: dict[str, list[dict]]):
    """lag_windows.json is a service-keyed map of sorted window rows with exact keys."""
    expected_keys = {
        "start_ms",
        "end_ms",
        "duration_ms",
    ...
    for blocks in windows.values():
        ...
        for block in blocks:
            ...
            # Handoff attenuation rounds UP per LOG-2252.
            assert block["adjusted_billable_duration_ms"] == max(
                block["billable_duration_ms"] - (-(-block["handoff_overlap_ms"] // 2)),
                0,
            )
            # Blackout attenuation rounds UP per LOG-2254; degrade below stays floored.
            ...
            # Debt credit rounds UP per LOG-2242; the layer divisors above stay floored.
            ...
            assert block["source_batch_ids"] == sorted(block["source_batch_ids"])
            assert isinstance(block["source_batch_ids"], list)
            assert all(isinstance(value, str) for value in block["source_batch_ids"])
...
def test_queue_required_fields_and_lengths(queue_rows: list[dict]):
    ...
    for row in queue_rows:
        ...
        assert len(row["batch_signature"]) == 12
...
def test_jsonl_compact_format():
    """retry_queue.jsonl uses compact JSON separators."""
...
def test_broken_snapshot_is_wrong():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        broken = Path(tmp) / "log_ship.py"
        out = Path(tmp) / "broken_out"
        shutil.copy(ORIGINAL_PIPELINE, broken)
        result = _run_pipeline(pipeline=broken, output_dir=out)
        assert result.returncode == 0, result.stderr
        broken_summary = _load_json(out / "shipment_summary.json")
        broken_windows = _load_json(out / "lag_windows.json")
        queue = _load_jsonl(out / "retry_queue.jsonl")
...
def test_pipeline_rerun_idempotent(summary: dict, windows: dict[str, list[dict]], queue_rows: list[dict]):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert result.returncode == 0, result.stderr
        assert _load_json(rerun / "shipment_summary.json") == summary
        assert _load_json(rerun / "lag_windows.json") == windows
        assert _load_jsonl(rerun / "retry_queue.jsonl") == queue_rows
...
def test_pipeline_supports_custom_output_dir():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert files == {"shipment_summary.json", "lag_windows.json", "retry_queue.jsonl"}
...
def test_cli_defaults_work_and_match_explicit_run():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert _load_json(SUMMARY_PATH) == _load_json(explicit_out / "shipment_summary.json")
        assert _load_json(WINDOWS_PATH) == _load_json(explicit_out / "lag_windows.json")
        assert _load_jsonl(QUEUE_PATH) == _load_jsonl(explicit_out / "retry_queue.jsonl")
...
def test_maintenance_source_path_affects_output():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = _run_pipeline(output_dir=out)
            assert result.returncode == 0, result.stderr
            summary = _load_json(out / "shipment_summary.json")
...
def test_policy_source_path_affects_output():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary_a = _load_json(out_a / "shipment_summary.json")
            queue_a = _load_jsonl(out_a / "retry_queue.jsonl")
            ...
            summary_b = _load_json(out_b / "shipment_summary.json")
            queue_b = _load_jsonl(out_b / "retry_queue.jsonl")
...
def test_maintenance_compaction_and_span_count_exercised():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert result.returncode == 0, result.stderr
            windows = _load_json(out / "lag_windows.json")
...
def test_dedupe_tie_break_chain_exercised():
    """Duplicate batch_id rows collapse to one row via the full governed tie-break chain.
    ...
    try:
        MAINTENANCE_PATH.write_text("[]\n")
        rows = [
            {"batch_id": "d1", "service": "payments", "start_ms": 100, "end_ms": 200, "severity": "major", "planned": True},
            {"batch_id": "d1", "service": "billing", "start_ms": 120, "end_ms": 200, "severity": "major", "planned": False},
            {"batch_id": "d1", "service": "auth", "start_ms": 130, "end_ms": 200, "severity": "critical", "planned": False},
            {"batch_id": "d1", "service": "billing", "start_ms": 130, "end_ms": 200, "severity": "critical", "planned": False},
        ]
        with tempfile.TemporaryDirectory() as tmp:
            in_path = Path(tmp) / "in.json"
            out = Path(tmp) / "out"
            ...
            block = windows["billing"][0]
            # LOG-2258 reverses the severity step: the LOWER rank wins the tie, so the
            ...
            assert block["source_batch_ids"] == ["d1"]
    finally:
        MAINTENANCE_PATH.write_text(original)
...
def test_planned_coercion_aliases_and_unknown_severity_exercised():
    ...
    try:
        MAINTENANCE_PATH.write_text("[]\n")
        rows = [
            {"batch_id": "p1", "service": "authentication", "start_ms": 10, "end_ms": 300, "severity": "major", "planned": "no"},
            {"batch_id": "p2", "service": "search-api", "start_ms": 20, "end_ms": 260, "severity": "weird", "planned": "yes"},
            {"batch_id": "p3", "service": "payments", "start_ms": 30, "end_ms": 280, "severity": "critical", "planned": 2},
        ]
        with tempfile.TemporaryDirectory() as tmp:
            in_path = Path(tmp) / "in.json"
            out = Path(tmp) / "out"
...
def test_handoff_compaction_and_scope_exercised():
    """Handoff windows compact per (service, severity scope) and match only in-scope batches.

    A window attenuates an batch only when its severity scope covers that batch, and the
...
def test_blackout_compaction_and_scope_exercised():
    """Blackout windows compact per (service, severity scope) and match only in-scope batches.
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # adjusted - ceil(190/3) = adjusted - 64 per LOG-2254 (floor gave 237)
...
def test_degrade_compaction_and_scope_exercised():
    """Degrade windows compact per (service, severity scope) and match only in-scope batches.
...
def test_debt_ledger_propagates_and_decays_between_windows():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # debt_out//80 + ceil(debt_in/120) = 8 + 4 per LOG-2248 (floored debt_in gave 11)
...
WRAPPER_PATH = Path("/usr/local/bin/ship-logs")
LOCK_PATH = Path("/var/lock/log-shipper.lock")
CRON_PATH = Path("/etc/cron.d/log-shipper")
CRON_LINE = (
    "*/5 * * * * svc-logship /usr/local/bin/ship-logs "
    "--input /app/data/batches.json --output-dir /app/output"
)


def test_service_account_provisioned():
    """The svc-logship system account exists with a non-interactive shell."""
    import pwd
...
def test_wrapper_installed_and_functional(tmp_path_factory):
    ...
    assert result.returncode == 0, result.stderr
    for name in ("shipment_summary.json", "lag_windows.json", "retry_queue.jsonl"):
...
def test_wrapper_honors_compile_lock(tmp_path_factory):
    ...
    try:
        ...
        assert not (out_dir / "shipment_summary.json").exists()
    finally:
...
def test_output_dir_ownership_and_mode():
    """/app/output is owned by svc-logship with mode 0750."""
    import pwd
...
def test_log_directory_ownership_and_mode():
    ...
    under svc-logship:svc-logship at mode 0750, since the scheduled compile writes there as that
    account and a world-writable log directory is a rollout defect.
    """
    import pwd

    log_dir = Path("/var/log/log-shipper")
    assert log_dir.is_dir(), "compile log directory is missing"
    info = log_dir.stat()
    svc = pwd.getpwnam("svc-logship")
    assert info.st_uid == svc.pw_uid and info.st_gid == svc.pw_gid, "log dir not owned by svc-logship"
...
def test_rollout_leftover_log_pruned():
    ...
    /var/log/log-shipper/compile.log.0 was left behind mid-compile and must be pruned during
    recovery rather than handed to the next rotation. The live compile.log is not a leftover and
    must remain in place.
    """
    assert not Path("/var/log/log-shipper/compile.log.0").exists(), "rollout leftover not pruned"
    assert Path("/var/log/log-shipper/compile.log").exists(), "live compile log was removed"


def test_logrotate_dropin_installed():
    """Rotation is configured to run as the service account, not as root.

    The drop-in at /etc/logrotate.d/log-shipper must exist at mode 0644 covering the compile
    log glob, and must carry the su/create directives that keep rotated files owned by
    svc-logship along with the documented retention and safety directives.
    """
