...
> **Change-review decision (2026-05-17 - LOG-2248)** Ilya: the debt-in half of the debt pressure score ROUNDS UP while the debt-out half does not: `debt_pressure_score = (debt_out_ms // 80) + ceil(debt_in_ms / 120)`. The 80 debt-out divisor is unchanged.
...
> **Change-review decision (2026-05-30 - LOG-2264)** Marta: degrade probe rounding, and the rounding map of record. The SEVERITY-scoped half of the degrade probe now ROUNDS UP (ceiling) while the all-scoped half keeps its floor: `degrade_pressure_score = (degrade_all_probe_ms // 34) + ceil(degrade_severity_probe_ms / 23) + degrade_segment_count`. In integer arithmetic ceil(x/23) is -(-x // 23). This revises the floored `degrade_severity_probe_ms // 23` written in LOG-2227, which is superseded on this point only: the probe range [end_ms-210, end_ms+1), the divisors 34 and 23, the all-scoped floor and the segment term are unchanged. This entry also supersedes LOG-2250 as the rounding map of record: rounding remains NON-uniform and no divisor's direction may be inferred from any other, but the degrade probe no longer stays floored on both halves — only the handoff probe does. Read each divisor's direction from its own governing decision.

### Review entry 0596 — rounding bench

> **Change-review decision (2026-06-02 - LOG-2266)** Ilya: rounding map of record, consolidated for the avoidance of doubt after repeated misreadings caused re-derivation errors downstream. This supersedes LOG-2264 as the map of record and states every governing rounding DIRECTION in one place; the divisor VALUES are unchanged and each still lives in that quantity's own governing entry, as do the dedupe tie-break, the interval/overlap and cross-service scope semantics, and every other non-rounding rule — this entry records directions only and changes nothing. Attenuation subtractions from billable time: handoff CEIL, blackout CEIL, degrade FLOOR. Debt ledger: the idle-gap decay is FLOOR and the debt credit added to the dispatchable duration is CEIL; the debt_out cap is a plain minimum. Debt pressure score: the debt-out half is FLOOR and the debt-in half is CEIL. Pressure probes, given as (all-scope half / severity-scope half): handoff FLOOR / FLOOR, blackout FLOOR / CEIL, degrade FLOOR / CEIL. Unit counts: suppress CEIL and boost CEIL, while handoff, blackout and degrade unit counts are FLOOR. Rounding remains NON-uniform; no direction here may be inferred for any quantity not named. The divisors themselves (for example the debt credit's 5, the blackout severity probe's 24, the degrade severity probe's 23, the attenuation 2/3/4) are as recorded in their governing entries and are NOT restated here.
